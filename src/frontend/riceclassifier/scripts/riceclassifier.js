 ...
 * @param {Object} options - UI references passed in for testing or DOM updates
 * @param {HTMLElement} options.resultDiv - Element to display results or errors
 * @param {HTMLElement} options.loadingDotContainer - Loading indicator container
 * @param {HTMLElement} options.inputPanel - Input panel element (used for animations)
 * @param {HTMLImageElement} options.uploadedImage - Image element where the uploaded image is shown
 */
async function handleFile(input, options = {}) {
    ...
    try {
        ...
        if (loadingDotContainer) loadingDotContainer.style.display = 'flex';
        if (inputPanel) inputPanel.classList.add('animate');

        if (uploadedImage) updateImage(base64Image, uploadedImage);  // Display the image immediately
        let base64ImageNoPrefix = base64Image.split(',')[1];
        await sendImageToLambda(base64ImageNoPrefix, resultDiv);

    } catch (error) {
        console.error("Error processing the file:", error);
    } finally {
        if (loadingDotContainer) loadingDotContainer.style.display = 'none';
        if (inputPanel) inputPanel.classList.remove('animate');
 ...
 * @param {HTMLImageElement} uploadedImage - The <img> element to update with the new image
 */
function updateImage(base64Image, uploadedImage) {
 ...
 * @param {HTMLElement} resultDiv - The element to update with classification results or errors
 */
async function sendImageToLambda(base64Image, resultDiv) {
    try {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    uploadArea.addEventListener('drop', (e) => {
        ...
        if (file) {
            handleFile(file, handleFileOptions);
        } else {
            const src = e.dataTransfer.getData("text/plain"); // Check for preset image src
            if (src) {
                handleFile(src, handleFileOptions); // Handle preset image URL
            } else {
